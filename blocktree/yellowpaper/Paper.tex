...
\definecolor{pagecolor}{rgb}{1,.85,0.95}
...
\title[The World Piecedie Fractal: An NFT Blocktree Fungible Token Mint]{The World NFPiece Fractal:\\A NFT-Blocktree Fungible Token Mint\\V0.0}
\author{
    to fund invention of\\
    \\
    World Piece Computers\\
    \& The Universal Peace Computer\\
    \\
        \emph{\tiny{blairmunroakusa}}\\
}
...
\begin{flushleft}
    \vspace*{-2cm}
    \small{\textbf{\textsc{SECURITY TAG:}} MDwwDQYJKoZIhvcNAQEBBQADKwAwKAIhAK2nk8DkKWyCVjRDHTXirIruvdmzdKOZ/ES3ZC/XQ4nVAgMBAAE=}\\
        \vspace*{0.1cm}

\end{flushleft}

\noindent
\tiny{To provide a way to validate the authenticity of all future CPFrac color-papers, and to create an immutable linear provenance despite nonlinear versionizing and progression, with each new publication will come a new public key plus the private key from the previous publication. Thus, this document series is equipped with an RSA-256 key-linked validation chain to authenticate operator BLAIRMUNROAKUSA as the approving author. To validate a document, the validator will use the current provided private key to generate the public key given by the previous document. The previous document location on redundant blockchains will be provided as a lookup to find this public key. Without the validation of this key-linked chain, any document published was not officially done so by BLAIRMUNROAKUSA. Once I figure out how to do it, these documents will be watermarked with the current key, and immortalized as a visual NFT on redundant public blockchains. This protocol may upgrade to a more sophisticated key generation method at any time, but that change will be specified in that particular piece. A piece will only be published in disagreement with the CPFrac mission if BLAIRMUNROAKUSA is operating under extreme duress or distress.}
\pagecolor{pagecolor}

\begin{abstract}
This \emph{\textbf{Pink Paper}} defines the formal World Non-Fungible Piece Fractal protocol, describing the implementation of a hashlinked, NFT-blocktree, distributed fungible token mint. The purpose of building this mint is to raise funds to initially capitalize the overarching project, manifesting as a new form of dynamic peace advocacy. This project is in its nascent stage, and this publication marks the official date of birth. At the highest level of abstraction, the purpose of this project is to create a new kind of ubiquitous Human object or construct called \emph{\textbf{the computational peace fractal}} or \emph{\textbf{CPFrac}}. The computational peace fractal will be defined in terms of a new computer architecture and a general stuff computer.\\
\\
\textbf{CPFrac MISSION: accelerate advent of global world peace in a safe, incentivized, and nonlinear manner, by inventing then implementing the computational peace fractal.}\\

This paper 

Creating the CPFrac will amount to a largescale social invention project. To fund kickoff, The World Non-Fungible Piece Fractal was chosen as the first nested phase in an effort to invent the \emph{infrastructure} for the computational peace fractal. The essence of the CPFrac revolves around the computer. Once funded and properly incentivized, the invention project will focus at first on inventing and installing individual and community \emph{\textbf{world piece computers}}. (For now it is enough know that these computers will be purposed to manage all the pieces within individual worlds, and there are as many worlds as there are individuals and communities.) Once enough world piece computers exist, they must be networked and integrated under a single operating system. (For now it is enough to know that this integration is meant to unify individual and community worlds plus their respective \emph{peace} processes into a singular global process sharing resources over a common inter-peacenet. This entity is the singular macroscopic form of the computational peace fractal, and is called The Universal Peace Computer, a global supercomputer dedicated to achieving, sustaining, and improving world peace. In its current state of art, The Universal Peace Computer exists as a simple network of Human brains aligned in common to the mission of the CPFrac project. This networking and integration will happen in a manner consisting at first as disjointed exponential progress toward peace within and between unrelated worlds, gradually culminating in a final quasi-steady-state of peace. This state should be achieved over time via a cascade of convergent explosive percolation events, each one unifying our disparate improving worlds into an ever cohesive whole (held together by The Universal Peace Computer).\\

This project, and its ensuing network of integrated brains, vow to take this prospect seriously, and far beyond the usual ‘don’t mind me; what more could I do beyond my niche of a part.’ If by now it is not clear, understand; existential threats loom over our species, and the best we seem to manage in response are a plurality of competing do-good causes and campaigns and isms, culminating in a billionaire space race to yank the eject lever. For those who prefer Earth and see things as they are, one thing is clear: if you believe we stand a chance at solving our existential problems \textbf{\emph{without first prioritizing world peace as a species}}, then respectfully, I believe you are a fool.\\
\\
Skipping to the bottom line:\\

The current economic status quo has inconceivable momentum. There is no changing course by the infinitesimal impulse of individuals and special interest groups, not even governmental not trans governmental policy alone. Besides, for every individual or group impulse aligned left, there is reliably an individual or group impulse aligned right. Don’t mention up, down, fore, back, in, out. Thus, despite our best individual (person or group) intentions, the net impulse on this massive economic object is usually nil, even if we disregard the infinitesimal nature of the efforts. Fortunately, this is not to fret, for we have learned some things by now. If we can’t beat, corral, confine a thing, well then in our experience we are left with no choice but to harness the thing, and it usually works to our benefit. We call this harness, a \emph{technology}.

Our immediate difficultly to contend with is the economic problem. Our current global status quo has an inconceivable momentum. Special interest groups are infinitesimal forces, and even then, there are usually competing infinitesimal forces anti-aligned to cancel out. This dilemma plays out on all stages, in all theaters. Our current economic system is not as equitable and just as it could be. It is inefficient and destructive. Yet, we cannot turn it off, turn it around, nor replace it on a whim.

Global circumstances and technological paradigms have aligned over time to set the stage for an inevitable emergent phase change in the state of \emph{Homo sapiens}. The infrastructure is nearly in place to provide ubiquitous fair access to the edifice of Human knowledge. Advances in molecular biology and genetics are quickly mapping the most intricate aspects of our chemical being. As network throughput increases, artificial intelligence is positioned as a tool to reliably help us interpret and manipulate continuous megalithic datasets representing ultra high dimensional complex phenomena such as Human behavior in the context of society, or even the planetary ecosystem. For several decades, progress in fusion research has been progressing at a rate comparable to the development of computers. This paragraph could fill a large book full of wonderful news about the technological state of Humanity; the point is to ask if we seem so fully equipped with brilliant technology, then why do things appear to deteriorate faster than they improve? Why is the culmination of all individual and community do-good efforts

This paper is divided into three colors, RGB.

Red introduces the subject on hand, incentivized computational peace. Blue outlines the
